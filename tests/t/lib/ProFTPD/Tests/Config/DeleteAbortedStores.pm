...
my $TESTS = {
  ...
  deleteabortedstores_hiddenstores_incomplete_dir_ok => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub deleteabortedstores_hiddenstores_incomplete_dir_ok {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  ...
  my $incomplete_dir = File::Spec->rel2abs("$tmpdir/Incomplete");
  unless (mkdir($incomplete_dir)) {
    die("Can't create directory $incomplete_dir: $!");
  }

  my $hidden_file = File::Spec->rel2abs("$tmpdir/.in.test.txt.");
  my $moved_file = File::Spec->rel2abs("$tmpdir/Incomplete/in.test.txt.");
  my $test_file = File::Spec->rel2abs("$tmpdir/test.txt");
  ...
  if ($pid) {
    eval {
      ...
      if (-f $hidden_file) {
        die("File $hidden_file exists unexpectedly; it should have been moved to Incomplete/");
      }

      unless (-f $moved_file) {
        die("File $moved_file does not exist as expected; aborted HiddenStores file was not moved to Incomplete/");
      }
    };
